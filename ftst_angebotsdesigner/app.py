...
def offers():
    ...
    try: data=BillomatClient(bid,key).list_offers(request.args.get("search",""))
    except Exception as e: return base("Billomat Fehler",f'<div class="card"><h1>Billomat-Fehler</h1><p>{e}</p></div>'),502
    rows=''.join(f'<tr><td>{o.get("offer_number") or o.get("number","")}</td><td>{o.get("date","")}</td><td>{o.get("title","")}</td><td>{money(o.get("total_gross"))}</td><td><a class="btn" href="{ingress("offer/"+str(o.get("id")))}">Öffnen</a></td></tr>' for o in data)
...
def offer(offer_id):
    ...
    except Exception as e:return base("Fehler",f'<div class="card"><h1>Fehler</h1><p>{e}</p></div>'),502
    return detail(o)
...
def make_pdf(o):
    ...
    p.setFillColorRGB(.08,.08,.08);p.rect(0,h-35*mm,w,35*mm,fill=1,stroke=0)
    p.setFillColorRGB(1,1,1);p.setFont("Helvetica-Bold",19);p.drawString(20*mm,h-18*mm,"FT SICHERHEITSTECHNIK")
    p.setFillColorRGB(.84,.05,.08);p.rect(0,h-38*mm,w,3*mm,fill=1,stroke=0)
    p.setFillColorRGB(.1,.1,.1);p.setFont("Helvetica-Bold",24);p.drawString(20*mm,h-58*mm,"Ihr persönliches Angebot")
    y=h-72*mm;p.setFont("Helvetica",10);p.setFillColorRGB(.2,.2,.2)
    c=o.get('client') or {}; p.drawString(20*mm,y,client_name(c));p.drawString(20*mm,y-5*mm,f"{c.get('street','')} · {c.get('zip','')} {c.get('city','')}")
    p.drawRightString(w-20*mm,y,f"Angebot {o.get('offer_number') or o.get('number','-')}");p.drawRightString(w-20*mm,y-5*mm,f"Datum {o.get('date','-')}")
    y-=18*mm;p.setFont("Helvetica-Bold",14);p.drawString(20*mm,y,o.get('title') or 'Projektübersicht');y-=8*mm;p.setFont("Helvetica",10)
    intro=(o.get('intro') or '').replace('\n',' ');p.drawString(20*mm,y,intro[:115]);y-=12*mm
    p.setFont("Helvetica-Bold",9);p.drawString(20*mm,y,"Pos.");p.drawString(35*mm,y,"Leistung / Artikel");p.drawString(125*mm,y,"Menge");p.drawString(150*mm,y,"Einzelpreis");p.drawRightString(w-20*mm,y,"Netto");y-=5*mm;p.line(20*mm,y,w-20*mm,y);y-=7*mm;p.setFont("Helvetica",9)
    for i in o.get('items',[]):
        title=str(i.get('title',''));desc=str(i.get('description','') or '');p.drawString(20*mm,y,str(i.get('position','')));p.drawString(35*mm,y,title[:50]);p.drawString(125*mm,y,f"{i.get('quantity','')} {i.get('unit','')}");p.drawString(150*mm,y,money(i.get('unit_price')));p.drawRightString(w-20*mm,y,money(i.get('total_net')));y-=8*mm
        if desc:y-=2*mm;p.setFont('Helvetica',7);p.drawString(35*mm,y,desc[:75]);p.setFont('Helvetica',9);y-=6*mm
        if y<35*mm:p.showPage();y=h-25*mm
    y-=4*mm;p.line(120*mm,y,w-20*mm,y);y-=8*mm;p.setFont('Helvetica-Bold',11);p.drawString(120*mm,y,"Netto");p.drawRightString(w-20*mm,y,money(o.get('total_net')));y-=7*mm;p.drawString(120*mm,y,"Gesamt");p.drawRightString(w-20*mm,y,money(o.get('total_gross')));p.setFont('Helvetica',7);p.drawString(20*mm,12*mm,"FT Sicherheitstechnik · Professionelle Sicherheitstechnik")
    p.save();out.seek(0);return out

@app.get("/offer/<offer_id>/pdf")
def offer_pdf(offer_id):
    if offer_id=='demo':
        o={"offer_number":"26-79","date":"12.06.2026","title":"Videoüberwachung – Pasa Firin","total_net":6452.40,"total_gross":7678.36,"client":{"name":"Pasa Firin","street":"G2 9","zip":"68159","city":"Mannheim"},"items":[]}
    else:
        bid=os.getenv('BILLOMAT_ID');key=os.getenv('BILLOMAT_API_KEY')
        if not bid or not key: abort(503)
        o=BillomatClient(bid,key).get_full_offer(offer_id)
    return send_file(make_pdf(o),mimetype='application/pdf',as_attachment=False,download_name=f"FTST-Angebot-{o.get('offer_number','')}.pdf")
