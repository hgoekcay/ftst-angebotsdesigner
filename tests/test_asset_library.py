...
def test_packaged_originals_and_ingress(client):
    ...
    for key in images:
        ...
        assert response.mimetype in ('image/jpeg', 'image/png')
...
def test_logo_default_and_explicit_disable_survive_restart(client):
    ...
    assert Path(materials.enrich({'id': '42'}, store)['logo_path']).is_file()
    assert client.post('/company', data={'logo': ''}).status_code == 302
    assert 'logo_path' not in materials.enrich({'id': '42'}, module.offer_store())
    assert client.post('/company', data={'logo': DEFAULT_LOGO}).status_code == 302
    assert 'logo_path' in materials.enrich({'id': '42'}, module.offer_store())
